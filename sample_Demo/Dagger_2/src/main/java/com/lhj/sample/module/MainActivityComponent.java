 ...
 *
 *
 * @Scope是需要成对存在的，在Module的Provide方法中使用了@Scope，
 * 那么对应的Component中也必须使用@Scope注解，当两边的@Scope名字一样时（比如同为@Singleton）,
 ...
 *
 * 注意：
 * @Singleton也是一个自定义@Scope，它的作用就像上面说的一样。
 * 但由于它是Dagger2中默认定义的，所以它比我们自定义Scope对了一个功能，
 * 就是编译检测，防止我们不规范的使用Scope注解，仅此而已。
 */

// 通常在Android中用来标记在App整个生命周期内存活的实例
//@Singleton // 表示只创建一次对象，但是这个一次只是在 Dagger2的生命周期中来说是只创建一次
@ActivityScoped // 这里必须标注@Scope注解　@ActivityScoped是自定义的@Scope，不能直接添加@Singleton
